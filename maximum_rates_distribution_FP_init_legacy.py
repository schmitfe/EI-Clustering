...
def main() -> None:
    ...
    if target_rep is None:
        if stored_rep is None:
            raise ValueError(
                "Parameter set must define R_Eplus or analysis metadata must record it. "
        ...
        if stored_rep is not None and abs(stored_rep - target_rep) > 1e-9:
            raise ValueError(
                f"Analysis folder {analysis_dir} already recorded R_Eplus={stored_rep}. Requested {target_rep} incompatible."
            )
        if stored_rep is None:
