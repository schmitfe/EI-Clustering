...
def psth_plot(spiketimes,binsize=2.,tlim = None,color = 'k',lw = 2.):
    ...
    bin_count = int((tlim[1] - tlim[0]) / float(binsize))
    bin_count = max(1, bin_count)
    bins = pylab.linspace(tlim[0], tlim[1], bin_count)
...
def std_plot(x,y,color):
    ...
    not_nan_inds = pylab.where(~pylab.isnan(mean))[0]
...
def percentile_plot(x,y,ps = [25,75],color='b'):
    ...
    not_nan_inds = pylab.where(~pylab.isnan(bottom))[0]
