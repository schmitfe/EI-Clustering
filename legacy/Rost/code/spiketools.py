...
def kernel_rate(spiketimes,kernel,tlim = None,dt=1.,pool = True):
    ...
    half_width = kwidth//2
    if half_width > 0:
        rates = rates[:,half_width:-half_width]
        time = time[half_width:-half_width]
...
def time_resolved_cv2(spiketimes,window = None,ot = 5.,tlim = None,pool = True,tstep = 1.,bessel_correction=False,minvals = 0,return_all = False):
